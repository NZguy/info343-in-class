...
function reducer(state, action){
    switch(action.type){
        case ADD_FAV_ACTION:
            // Object.assign Creates a new js object, copying all data from all 
            // parameters, essentially a clone operation
            // Concat adds the parameter to the end of the object operated on
            return Object.assign({}, {favorites: state.favorites.concat(action.item)}); 
        case REMOVE_FAV_ACTION:
            // When filter returns false, the item is filtered out
...
// .filter, .find, .includes may be useful functions

// if you want to save the redux state you can use firebase or local storage
// local storage is a new feature of html 5, local storage can only use string so
